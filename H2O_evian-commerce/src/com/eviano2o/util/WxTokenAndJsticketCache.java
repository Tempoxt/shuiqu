...
*/
	...
	public static String getJsapi_ticket(String appId) {
		if(accessTokenAndJsTicketMap == null)
			accessTokenAndJsTicketMap = new Hashtable<String, AccessTokenAndJsTicket>();
			
		if(StringUtils.isEmpty(appId))return "";
		
		if(!accessTokenAndJsTicketMap.containsKey(appId)||accessTokenAndJsTicketMap.get(appId)==null){
			AccessTokenAndJsTicket entity = new AccessTokenAndJsTicket();
			entity.setAppId(appId);
		...
		}else if(accessTokenAndJsTicketMap==null){
			logger.error("accessTokenAndJsTicketMap = null");
		}else if(accessTokenAndJsTicketMap.get(appId)==null){
			logger.error("accessTokenAndJsTicketMap.get(appId) = null");
		}
