...
public class ImageWaterMakeUtil {
	private static final Logger logger = LoggerFactory.getLogger(ImageWaterMakeUtil.class);
	 ...
	 *            水印图片旋转角度
	 */
	public static Boolean markImageByIcon(String iconPath, String srcImgPath, String targerPath, Integer degree, String pressText) {
		...
		try {
			...
			// 表示水印图片的位置
			g.drawImage(img, 48, 80, null);
			g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER));

			g.setColor(Color.white);
			g.setFont(new Font("黑体", 36, 36));
			g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP, alpha));
			// 在指定坐标绘制水印文字
			g.drawString(pressText, 180, 110);
			...
			logger.info("生成海报错误：" + e.toString() + "   iconPath：" + iconPath + "   srcImgPath：" + srcImgPath + "   targerPath：" + targerPath + "   pressText：" + pressText);
			e.printStackTrace();
			return false;
		} finally {
			try {
	 ...
	 *            水印图片旋转角度
	 */
	public static Boolean evianShareTemplate(String srcImgPath, String targerPath, List<ShareTemplatePicsModel> pictrues, List<ShareTemplateTextModel> texts) {
		OutputStream os = null;
		try {
			Image srcImg = ImageIO.read(new File(srcImgPath));
			BufferedImage buffImg = new BufferedImage(srcImg.getWidth(null), srcImg.getHeight(null), BufferedImage.TYPE_INT_RGB);
			...
			if (pictrues != null && pictrues.size() > 0) {
				for (ShareTemplatePicsModel pic : pictrues) {
					// 水印图象的路径 水印一般为gif或者png的，这样可设置透明度
					ImageIcon imgIcon = new ImageIcon(pic.getPicPath());
					// 得到Image对象。
					Image img = imgIcon.getImage();
					float alpha = pic.getAlpha(); // 透明度
					g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP, alpha));
					// 消除文字锯齿
					g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
					// 表示水印图片的位置
					g.drawImage(img, pic.getPicX(), pic.getPicY(), null);
					g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER));
				}
			}

			if (texts != null && texts.size() > 0) {
				for (ShareTemplateTextModel t : texts) {
					g.setColor(t.getFontColor());
					g.setFont(new Font(t.getFontName(), t.getFontStyle(), t.getFontSize()));
					g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP, t.getAlpha()));
					// 在指定坐标绘制水印文字
					g.drawString(t.getTextContent(), t.getFontX(), t.getFontY());
				}
			}
			...
			logger.info("生成海报错误：" + e.toString() + "   pictrues：" + pictrues.toString() + "   srcImgPath：" + srcImgPath + "   targerPath：" + targerPath + "   texts：" + texts.toString());
			e.printStackTrace();
			return false;
		} finally {
			try {
	 ...
	 * 测试OK // 2-切割图像： // 方法一：按指定起点坐标和宽高切割 ImageUtils.cut("e:/abc.jpg",
	 * "e:/abc_cut.jpg", 0, 0, 400, 400);// 测试OK // 方法二：指定切片的行数和列数
	 * ImageUtils.cut2("e:/abc.jpg", "e:/", 2, 2);// 测试OK // 方法三：指定切片的宽度和高度
	 * ImageUtils.cut3("e:/abc.jpg", "e:/", 300, 300);// 测试OK // 3-图像类型转换：
	...
	public final static void scale2(String srcImageFile, String result, int height, int width, boolean bb) {
		try {
			...
			// e.printStackTrace();
			logger.info("缩略图失败，图片来源："+srcImageFile    +"   保存地址："+result + "   "+ ex.getMessage());
		}
	}
