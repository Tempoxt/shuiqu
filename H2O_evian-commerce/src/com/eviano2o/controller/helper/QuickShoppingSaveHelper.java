...
public class QuickShoppingSaveHelper extends BaseControllerHelper {
	private static final Logger logger = LoggerFactory.getLogger(QuickShoppingSaveHelper.class);
	
	public QuickShoppingSaveHelper(Model model, HttpServletRequest request) {
		super(model, request);
	}
	...
	public void Init() {
		...
		//System.out.println(quickSpJson);
		if(quickSpJson.isEmpty()){
	...
	private void checkUserSubmitSp(List<QuickShoppingListModel> userSubmitSp, List<QuickShoppingListModel> userQuickSp){
			...
			for(QuickShoppingListModel qsp : userQuickSp){
				...
				if(ssp.getShopId().intValue() == qsp.getShopId().intValue()){
					...
					for(QuickShoppingGoods shopSSP : ssp.getHabits()){
						for(QuickShoppingGoods shopQSP : qsp.getHabits()){
							if(shopSSP.getPid().intValue() == shopQSP.getPid().intValue()){
								...
								if(shopSSP.getBuyType().intValue() == 1){
									//totalMoney = shopQSP.getNumber() * shopQSP.getVipPrice();
									totalShopMoney = DoubleFormatUtil.sum(totalShopMoney, shopQSP.getNumber() * shopQSP.getVipPrice());
								}
					...
					if(carOrderShopSP.size() > 0){
						...
						curShop.setPrivilegeId(0);
						curShop.setActivityType(0);
						curShop.setCode_no("");
						curShop.setCode_money(0.00);
						curShop.setDiscountMoney(0.00);
		...
		if(userCheckSp.size() > 0){
			...
			//logger.info("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"+_result);
		}
		else
			_result = formatJsonResult("E90000", "购物车中没有商品");
	...
	private String checkYuYueTime(String submitTime){
		if(StringUtils.isEmpty(submitTime) || submitTime.split(" ").length != 2){
			//System.out.println(submitTime + " | " + submitTime.split(" ").length);
			return "";
		}
		String date = "";
