...
public abstract class BaseControllerHelper{
	...
	public BaseControllerHelper(Model model, HttpServletRequest request){
		...
		_request = request;
		getSysParamMap();
		WxJsTicketUtil.sign(_model, currentUrl(), getSessionWXAppId());
		setShareParam();
		getWechatInfo();
	}
	...
	/** 公众号信息 缓存 */
	protected Map<String, e_enterprise_wechatliteapp> getWechatInfo() {
		// System.out.println("缓存："+SysParamCache.isNullObj());
		if (SysParamCache.getWechatLiteapp() == null || SysParamCache.getWechatLiteapp().size()==0) {
			String dataStr = weiXinService.getAllPublicAccount();
			if (StringUtils.isEmpty(dataStr)) {
				logger.error("[ex:{}]", new Object[] { "获取公众号信息失败" });
				return null;
			}
			
			JSONObject objJson = JSONObject.fromObject(dataStr);
			if (objJson.getString("code").equals("E00000") && objJson.getJSONObject("data") != null && objJson.getJSONObject("data").getInt("count") > 0) {
				List<e_enterprise_wechatliteapp> wechatList = new Gson().fromJson(objJson.getJSONObject("data").getString("publics"), new TypeToken<List<e_enterprise_wechatliteapp>>() { }.getType());
				if(wechatList == null || wechatList.size()==0){
					logger.error("============================="+objJson.getJSONObject("data").getString("publics"));
				}
				SysParamCache.setWechatLiteapp(null);
				Map<String, e_enterprise_wechatliteapp> resultMap = new HashMap<String, e_enterprise_wechatliteapp>();
				for (e_enterprise_wechatliteapp p : wechatList) {
					resultMap.put(p.getAppId(), p);
				}
				SysParamCache.setWechatLiteapp(resultMap);
			}
		}
		return SysParamCache.getWechatLiteapp();
	}
	...
	/** 微信关注用户资料下载 */
	public void getWXUserInfo(String openId, String qrscene, Integer shareId, String accessToken){
		new DownloadWXUserInfo(_request, accessToken, openId, qrscene, shareId, _request.getSession().getServletContext().getRealPath("/"), getSessionWXAppId()).doDownload(0);
	...
	public void getWXUserInfoNoDownload(String openId, String qrscene, Integer shareId){
		...
		if(!StringUtils.isEmpty(isDown)){
			...
			if(j.has("code") && j.getString("code").equals("E00000") && j.getJSONObject("data").getBoolean("exists") == true){
				...
				new DownloadWXUserInfo(_request, WxTokenAndJsticketCache.getAccess_token(getSessionWXAppId()), openId, qrscene, shareId, _request.getSession().getServletContext().getRealPath("/"), getSessionWXAppId()).doDownload(0);
