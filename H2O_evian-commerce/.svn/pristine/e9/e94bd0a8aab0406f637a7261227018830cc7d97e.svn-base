...
public class URLConnectionUtil {
    ...
    public static String postBackstageApi(String url, List<BasicNameValuePair> params) {
		...
		String param = basicNameValuePairToString(params);
        try {
            URL realUrl = new URL(url);
            // 打开和URL之间的连接
            ...
            conn.setRequestProperty("contentType", "plain/text; charset=UTF-8");
            // 获取URLConnection对象对应的输出流
        	...
        	logger.info("发送 POST 请求出现异常！"+e + "          url:"+url+"             param:"+param);
            e.printStackTrace();
        }
        //使用finally块来关闭输出流、输入流
        finally{
	...
	private static String basicNameValuePairToString(List<BasicNameValuePair> list){
		...
		for(BasicNameValuePair cur: list){
			result += (StringUtils.isEmpty(result) ? "" : "&") + cur.getName() + "=" + cur.getValue();
		}
		return result;
	}
}
